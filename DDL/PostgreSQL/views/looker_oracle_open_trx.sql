...
CREATE OR REPLACE VIEW mon.looker_oracle_open_trx AS
WITH LatestTrxData AS (
  ...
  FROM
    mon.striim_mon_oracle_open_trx
  GROUP BY
...
SELECT
  ...
  EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - ot.montimestamp)) / 60 as transaction_age_minutes,
  ROUND(EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - ot.montimestamp)) / 3600.0, 2) as transaction_age_hours,
  -- Transaction health status based on age
  CASE
    WHEN ot.montimestamp IS NULL THEN 'UNKNOWN'
    WHEN EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - ot.montimestamp)) / 60 <= 30 THEN 'NORMAL'
    WHEN EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - ot.montimestamp)) / 60 <= 120 THEN 'AGED'
    ELSE 'LONG_RUNNING'
  END as transaction_status
FROM
  mon.striim_mon_oracle_open_trx ot
INNER JOIN
  LatestTrxData ltd
