...
-- Consumer: mon.update_alert_thresholds() upserts from this view

CREATE OR REPLACE VIEW mon.StriimIntelligentThresholdRecommendations AS
WITH
...
AppMetadata AS (
  SELECT
    ...
    CASE
      ...
      WHEN (abm.checkpoint_records::FLOAT / abm.total_records) > 0.5 AND abm.avg_input > 100 AND abm.avg_output > 100 THEN TRUE
...
ThresholdComparison AS (
  ...
  WHERE
    am.last_seen >= CURRENT_TIMESTAMP - INTERVAL '7 days'  -- Only active apps
),
...
WHERE is_likely_cdc_app = TRUE
  AND (is_new_app OR should_update_terminated OR should_update_backpressure
       OR should_update_checkpoint OR should_update_lee
       OR should_update_largebatches OR should_update_queuedbatches OR should_update_sourceidle);
