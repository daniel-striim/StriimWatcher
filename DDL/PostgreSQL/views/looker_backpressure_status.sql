...
CREATE OR REPLACE VIEW mon.looker_backpressure_status AS
...
SELECT
  ...
  EXTRACT(EPOCH FROM (cb.batchdate - bd.backpressure_started)) / 60 as duration_backpressured_minutes,
  ROUND(EXTRACT(EPOCH FROM (cb.batchdate - bd.backpressure_started)) / 3600.0, 2) as duration_backpressured_hours,
  cb.totalInput,
  cb.totalOutput,
  cb.checkpointStatus,
  cb.latestActivity
