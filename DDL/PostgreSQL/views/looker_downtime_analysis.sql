...
CREATE OR REPLACE VIEW mon.looker_downtime_analysis AS
SELECT
  da.appName,
...
FROM
  mon.get_app_downtime_analysis(30) da
LEFT JOIN
  mon.latest_known_deployments lkd
  ON da.appName = lkd.appName
LEFT JOIN (
  SELECT DISTINCT rh.clusterName, ad.appName
  FROM mon.striim_mon_appdetail ad
  INNER JOIN mon.striim_mon_table_runhistory rh ON ad.batchdate = rh.batchdate
  INNER JOIN (SELECT MAX(batchdate) as max_batchdate FROM mon.striim_mon_table_runhistory) mb
