-- View: StriimIntelligentThresholdRecommendations
-- Purpose: Analyze historical application patterns to create data-driven default alert thresholds
-- Approach: Use statistical analysis (mean + 1 standard deviation) to set reasonable defaults
-- UPSERT Logic: Only flag a threshold for update if it differs by more than 20% from the current value
-- Block Size Logic: Round thresholds to configured block sizes (e.g., 60-minute blocks)
-- Consumer: `striim_watcher_metadata.update_alert_thresholds`() MERGEs from this view

CREATE OR REPLACE VIEW `striim_watcher_metadata.StriimIntelligentThresholdRecommendations` AS
...
ThresholdConfiguration AS (
  SELECT
    ...
    15 as terminated_min_minutes,
    30 as terminated_max_minutes,
    15 as terminated_block_size_minutes,

    -- Backpressure Thresholds
    60 as backpressure_min_minutes,
    120 as backpressure_max_minutes,
    30 as backpressure_block_size_minutes,

    -- Checkpoint Thresholds
    60 as checkpoint_min_minutes,
    120 as checkpoint_max_minutes,
    30 as checkpoint_block_size_minutes,
    ...
    524288000 as largebatches_min_bytes,    -- 500MB
    1048576000 as largebatches_max_bytes,   -- 1000MB
...
AppMetadata AS (
  ...
  SELECT
    ...
    CASE
      -- Rule 1: If "ILA_" is in the name (namespace), it is NOT CDC
      WHEN STRPOS(UPPER(abm.appName), 'ILA_') > 0 THEN FALSE
      ...
      WHEN (abm.checkpoint_records / abm.total_records) > 0.5 AND abm.avg_input > 100 AND abm.avg_output > 100 THEN TRUE

      -- Fallback: High activity apps are likely CDC
      WHEN abm.avg_input > 100 AND abm.avg_output > 100 THEN TRUE
...
-- Step 8: Generate summary and a per-app MERGE statement (for reference/debugging)
RecommendationSummary AS (
  SELECT
    ...
    current_terminated_threshold, new_terminated_threshold, should_update_terminated,
    current_backpressure_threshold, new_backpressure_threshold, should_update_backpressure,
    current_checkpoint_threshold, new_checkpoint_threshold, should_update_checkpoint,
    current_lee_threshold, new_lee_threshold, should_update_lee,
    current_largebatches_threshold, new_largebatches_threshold, should_update_largebatches,
    current_queuedbatches_threshold, new_queuedbatches_threshold, should_update_queuedbatches,
    current_sourceidle_threshold, new_sourceidle_threshold, should_update_sourceidle,
...
SELECT
  ...
  confidence_level,
  is_new_app,
  current_terminated_threshold, new_terminated_threshold,
  current_backpressure_threshold, new_backpressure_threshold,
  current_checkpoint_threshold, new_checkpoint_threshold,
  current_lee_threshold, new_lee_threshold,
  current_largebatches_threshold, new_largebatches_threshold,
  current_queuedbatches_threshold, new_queuedbatches_threshold,
  current_sourceidle_threshold, new_sourceidle_threshold,
  historical_downtime_events,
  max_historical_downtime_minutes,

  -- Reference-only MERGE statement text (not executed from the view itself)
  CASE
    WHEN needs_update THEN
      FORMAT("""
...
WHEN NOT MATCHED THEN INSERT (
  ...
  %d, %d, %d, %d, %d, %d, %d, FALSE, TRUE
    ...
    ELSE
      FORMAT("-- No significant changes for %s (all thresholds within configured tolerance)", appName)
  END as merge_statement
...
ORDER BY
  needs_update DESC,         -- Apps needing updates first
