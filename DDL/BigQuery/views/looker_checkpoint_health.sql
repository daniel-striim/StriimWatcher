...
CREATE OR REPLACE VIEW `striim_watcher_metadata.looker_checkpoint_health` AS
...
LatestCheckpoints AS (
  ...
  FROM
    `striim_watcher_metadata.striim_mon_checkpoint_history`
  WHERE
    batchdate >= TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL 24 HOUR)
  GROUP BY
    appName
),
CheckpointFrequency AS (
  ...
  FROM
    `striim_watcher_metadata.striim_mon_checkpoint_history`
  WHERE
    checkpointRecordedTime >= TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL 1 HOUR)
  GROUP BY
    appName
)
SELECT
  ...
  TIMESTAMP_DIFF(CURRENT_TIMESTAMP(), lc.last_checkpoint_time, MINUTE) as minutes_since_last_checkpoint,
  ...
  CASE
    ...
    WHEN TIMESTAMP_DIFF(CURRENT_TIMESTAMP(), lc.last_checkpoint_time, MINUTE) > 60 THEN 'STALE'
