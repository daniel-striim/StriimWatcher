...
| `OperationName` | Always `INSERT` — StriimWatcherV4A never updates or deletes rows |
...
**Use cases:** confirming StriimWatcherV4A is actually running, sizing `Repeat In Seconds` against `runtimeDurationMS`, and as the join key for every other table.
...
> **Note:** CPU rate fields throughout StriimWatcherV4A are reported per-core, not as a percentage of total machine capacity — an 8-core system can show up to 800%.
...
One row per source-target pair with latency data. StriimWatcherV4A excludes itself from this table (no self-referential noise).
...
**Use cases:** configuration audit trail and change history — who/what changed and when, for compliance or for tracking down unexpected behavior changes. This history is kept in memory only and does not survive a StriimWatcherV4A restart.
